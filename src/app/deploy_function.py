...
from src.models.components.my_resnet import ResNet, Residual_Block
...
mtcnn = MTCNN(thresholds= [0.7, 0.7, 0.8] ,keep_all=True, device = mps_device) ##0.7 0.7 0.8
...
def apply_filter_on_image(image, filter_name, isFirstFrame = True, optical_flow = optical_flow, isVideo = False):
    print(isFirstFrame)
    filter = filters_config[filter_name]
    frame = cv2.cvtColor(image, cv2.COLOR_BGR2RGB) if isVideo==False else image
    ...
    if boxes is not None:
        ...
        for j, face in enumerate(faces):
            ...
            if isFirstFrame:
                optical_flow['points2Prev'] = np.array(landmarks, np.float32)
                optical_flow['img2GrayPrev'] = np.copy(img2Gray)
                isFirstFrame = False
                # print("CCCCC")
            
            points2Prev = optical_flow['points2Prev']
            img2GrayPrev = optical_flow['img2GrayPrev']
            ...
            optical_flow['points2Prev'] = np.array(landmarks, np.float32)
            optical_flow['img2GrayPrev'] = img2Gray
    ...
    frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR) if isVideo==False else frame
    return frame


def apply_filter_on_video(source, filter_name = "anonymous")->None:
    ...
    output_path = '/Users/tiendzung/Project/facial_landmarks-wandb/src/app/processed_video.mp4'
    out = cv2.VideoWriter(output_path, fourcc, fps, (cap_width, cap_height))
    cnt = 0
    while cap.isOpened():
        # cur_time = time.time()
        ...
        if isSuccess:
            if cnt % 10 == 0:
                frame = apply_filter_on_image(frame, filter_name, isFirstFrame = (cnt==0), optical_flow = optical_flow, isVideo=True)
                out.write(frame)
                old_frame = frame
            else:
                out.write(old_frame)
...
def apply_filter_on_webcam(filter_name = "anonymous"):
    # prepare video capture
